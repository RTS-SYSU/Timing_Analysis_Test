...
struct BlockInfo {
  ...
  BlockInfo(unsigned addr, unsigned cnt, int a,
    ...
    this->cl = c;
    this->cap = std::max(L2assoc-cs_size,0);
  }
  ...
  struct CR{
    ...
    std::vector<int> OutNumber_PersistentBlock(UR ur){
      ...
      for(auto block:PersistentBlock){
          int sum=0, L=0, R=0, ans=0, cap=block.cap, cnt=block.cnt;
          for(auto block:ur)
              sum+=block.cnt;
          R=std::min(cnt, sum/cap);   ///////////////////////////////////////////////////////
          while(L<=R){
              int mid=L+R>>1, temp=0;
              for(auto block:ur)
                  temp+=std::min(mid, block.cnt);
              if(temp>=cap*mid)   L=mid, ans=mid;
              else    R=mid;
          }
    ...
    void updataCrUr(std::vector<int>PersistOut, UR ur, UR &NextUr){
      ...
      for(int i=0;i<PersistentBlock.size();++i){
          ...
          PersistentBlock[i].cnt -= num;
      }
      for(auto block:ur){
          if(block.cnt>mxOut)   // 先不考虑下一个ur和上一个ur有相同address的block的情况
            block.cnt=1, NextUr.push_back(block);
      }
    }
  };
    ...
    CRs(Ceop ceop){   
      ...
      for (int i = 0; i < ceop.size(); ++i) {
          for (auto block : ceop[i]) { 
              int cnt = block.cnt, age = block.age, cap=block.cap;
              unsigned addr=block.address;

              // 持久性块单独处理（要求不存在cap <= 0的块）   ///////////////////////////////////
              if(cnt>1)     PersistBlockInURs.push_back(block);
              // 检查是否成对出现且不在缓存外
              else if (LastURforBlock.count(addr) && LastURforBlock[addr] + 1 < i && age >= 0) { 
      ...
      for(auto cr:crs){
          ...
          for (int prevState = 0; prevState < lastDp.size(); ++prevState) { 
              ...
              for (int startUR = 0; startUR < Urs.size(); ++startUR) {
                  ...
                  // 模拟干扰任务执行
                  for (int curUR = startUR; curUR < Urs.urs.size(); ++curUR) {
                      totalOut += cr2.OutNumber_NotPersistBlock(curOut, Urs.urs[curUR]);
                      std::vector<int> PersistOut = cr2.OutNumber_PersistentBlock(Urs.urs[curUR]);
                      for(auto val:PersistOut)    totalOut += val;
                      if(curUR + 1 < Urs.urs.size())  // 只要不是最后一个UR就需要使用遗产
                          cr2.updataCrUr(PersistOut, Urs.urs[curUR], Urs.urs[curUR+1]);
