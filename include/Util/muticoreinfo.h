...
private:
  ...
  std::map<unsigned, std::map<std::string, 
    std::map<const llvm::MachineInstr*, 
      TimingAnalysisPass::dom::cache::Classification>>> mi_class; // aborted
  std::map<unsigned, std::map<std::string, // 先存下分类信息
    std::map<CtxMI, TimingAnalysisPass::dom::cache::Classification>>> ctxmi_class;
  std::map<unsigned, std::map<std::string, // core, function, ctxmi -> xclass
    std::map<CtxMI, X_Class>>> ctxmi_xclass;
  unsigned cur_core;
  std::string cur_func;
...
public:
  ...
  // 在拥有Class之后，此函数计算X(即执行次数)，需要cur_core core_func. FIXME：以CtxMI标识
  void getXClass(){
