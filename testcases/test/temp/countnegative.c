...
void countnegative_initialize(matrix Array) {
  ...
  _Pragma("loopbound min 20 max 20") for (OuterIndex = 0; OuterIndex <20;
                                          OuterIndex++)
...
void countnegative_sum(matrix Array) {
  ...
  _Pragma("loopbound min 20 max 20") for (Outer = 0; Outer < 20; Outer++)
      _Pragma(
          "loopbound min 20 max 20") for (Inner = 0; Inner < 20;
